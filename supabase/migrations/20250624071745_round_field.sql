/*
  # Complete database schema setup with proper error handling

  1. New Tables
    - `users` - User profiles linked to auth.users
    - `products` - Product database with pricing, categories, stock
    - `invoices` - Main invoice records
    - `invoice_items` - Line items for invoices
    - `tax_rates` - Tax rates applied to invoices

  2. Security
    - Enable RLS on all tables
    - Add policies for user data isolation (with proper cleanup)
    - Create trigger for automatic user profile creation

  3. Features
    - Product management with categories, SKU, stock tracking
    - Invoice management with multiple templates and customization
    - Tax and discount calculations
    - Payment QR code support
*/
