    ...
    return (
      <div className="bg-black text-white relative overflow-hidden\" style={{ 
        ...
        <div className="absolute inset-0 opacity-10">
          ...
          {/* Circuit Lines */}
          <div className="absolute top-10 left-10 w-40 h-1 bg-gradient-to-r from-cyan-400 to-transparent"></div>
          <div className="absolute top-10 left-10 w-1 h-40 bg-gradient-to-b from-cyan-400 to-transparent"></div>
          <div className="absolute bottom-10 right-10 w-40 h-1 bg-gradient-to-l from-cyan-400 to-transparent"></div>
          <div className="absolute bottom-10 right-10 w-1 h-40 bg-gradient-to-t from-cyan-400 to-transparent"></div>
          
          {/* Hexagonal Elements */}
          <div className="absolute top-20 right-20 w-16 h-16 border border-cyan-400 transform rotate-45"></div>
          <div className="absolute bottom-20 left-20 w-12 h-12 border border-cyan-400 transform rotate-45"></div>
        </div>

        <div className="relative z-10 p-8">
          {/* Tech Header */}
          <div className="border-b border-cyan-400 pb-8 mb-8 relative">
            ...
            <div className="flex justify-between items-start">
              <div className="flex items-center space-x-6">
                {invoice.company.logo && (
                  <div className="relative">
                    ...
                    <div className="relative bg-gray-900 p-4 rounded-full border border-cyan-400">
                      <img
                ...
                <div>
                  <h1 className="text-4xl font-bold mb-2 bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent">
                    {invoice.company.name || 'TECH CORP'}
                  </h1>
              ...
              <div className="text-right">
                <div className="bg-gradient-to-r from-cyan-400 to-blue-500 text-black px-6 py-3 rounded-lg font-bold text-2xl mb-4">
                  INVOICE
                </div>
                <div className="bg-gray-900 border border-cyan-400 px-4 py-2 rounded-lg">
                  <span className="text-cyan-400 font-mono text-lg">#{invoice.number}</span>
                </div>
                <div className="mt-4 text-cyan-300 text-sm">
                  <div>🚀 Status: <span className="text-white font-semibold">{invoice.status.toUpperCase()}</span></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-8 mb-8">
            {/* Client Info */}
            <div className="bg-gray-900 bg-opacity-50 border border-cyan-400 rounded-lg p-6 backdrop-blur-sm">
              <div className="flex items-center mb-4">
                <div className="w-3 h-3 bg-cyan-400 rounded-full mr-3 animate-pulse"></div>
                <h3 className="text-xl font-bold text-cyan-400 uppercase tracking-wide">CLIENT DATA</h3>
              </div>
              <div className="space-y-3 text-gray-300">
                {invoice.client.name && (
                  <div className="flex items-center">
                    <span className="text-cyan-400 mr-2">👤</span>
                    <span className="font-semibold text-white">{invoice.client.name}</span>
                  </div>
                )}
            ...
            <div className="bg-gray-900 bg-opacity-50 border border-cyan-400 rounded-lg p-6 backdrop-blur-sm">
              <div className="flex items-center mb-4">
                <div className="w-3 h-3 bg-cyan-400 rounded-full mr-3 animate-pulse"></div>
                <h3 className="text-xl font-bold text-cyan-400 uppercase tracking-wide">SYSTEM INFO</h3>
              </div>
              <div className="space-y-3">
                <div className="flex justify-between">
                  <span className="text-cyan-400">⏰ Issue Date:</span>
                  <span className="text-white font-mono">{formatDate(invoice.issueDate)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-cyan-400">⚡ Due Date:</span>
                  <span className="text-white font-mono">{formatDate(invoice.dueDate)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-cyan-400">💱 Currency:</span>
                  <span className="text-white font-mono">{invoice.currency}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Tech Table */}
          <div className="bg-gray-900 bg-opacity-50 border border-cyan-400 rounded-lg overflow-hidden mb-8 backdrop-blur-sm">
            <div className="bg-gradient-to-r from-cyan-400 to-blue-500 text-black p-4">
            ...
            <div className="divide-y divide-cyan-400 divide-opacity-30">
              {invoice.items.map((item, index) => (
                <div 
                  key={item.id} 
                  className={`grid grid-cols-12 gap-4 p-4 ${index % 2 === 0 ? 'bg-gray-800 bg-opacity-30' : 'bg-gray-900 bg-opacity-30'}`}
                >
                  <div className="col-span-6 text-gray-300">
                    <span className="text-cyan-400 mr-2">▶</span>
                    {item.description || 'System Component'}
                  </div>
                  <div className="col-span-2 text-center text-white font-mono">{item.quantity}</div>
                  <div className="col-span-2 text-right text-white font-mono">{currencySymbol}{item.rate.toFixed(2)}</div>
                  <div className="col-span-2 text-right font-bold text-cyan-400 font-mono">{currencySymbol}{(item.quantity * item.rate).toFixed(2)}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex justify-end mb-8">
            <div className="bg-gray-900 bg-opacity-70 border border-cyan-400 rounded-lg p-6 backdrop-blur-sm min-w-80">
              <div className="flex items-center mb-4">
                <div className="w-3 h-3 bg-cyan-400 rounded-full mr-3 animate-pulse"></div>
                <h3 className="text-xl font-bold text-cyan-400 uppercase tracking-wide">CALCULATION MATRIX</h3>
              </div>
              <div className="space-y-3 font-mono">
                ...
                {totals.taxAmount > 0 && (
                  <div className="flex justify-between text-gray-300">
                    <span>🏛️ Tax:</span>
                    <span className="text-white">{currencySymbol}{totals.taxAmount.toFixed(2)}</span>
                  </div>
                )}
                <div className="border-t border-cyan-400 pt-3">
                  <div className="flex justify-between text-2xl font-bold">
                    <span className="text-cyan-400">⚡ TOTAL:</span>
                    <span className="text-white bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent">
          ...
          {invoice.paymentInfo?.qrCode && (
            <div className="flex justify-center mb-8">
              <div className="bg-gray-900 bg-opacity-70 border border-cyan-400 rounded-lg p-6 backdrop-blur-sm text-center">
                <div className="flex items-center justify-center mb-4">
                  <div className="w-3 h-3 bg-cyan-400 rounded-full mr-3 animate-pulse"></div>
                  <h3 className="text-lg font-bold text-cyan-400 uppercase tracking-wide">PAYMENT PORTAL</h3>
                </div>
                ...
                <p className="text-cyan-300 text-sm mt-2">🔗 Scan to initiate payment protocol</p>
              </div>
            </div>
          )}

          {invoice.notes && (
            <div className="bg-gray-900 bg-opacity-50 border border-cyan-400 rounded-lg p-6 backdrop-blur-sm mb-8">
              <div className="flex items-center mb-4">
                <div className="w-3 h-3 bg-cyan-400 rounded-full mr-3 animate-pulse"></div>
                <h3 className="text-xl font-bold text-cyan-400 uppercase tracking-wide">SYSTEM NOTES</h3>
              </div>
        ...
        {invoice.showFooter && (
          <div className="border-t border-cyan-400 bg-gray-900 bg-opacity-50 text-center py-6">
            <div className="text-cyan-400 text-sm font-mono">
              🤖 Generated by Invoice Beautifier • Advanced Invoice System v2.0
            </div>
          </div>
        )}
      </div>
  ...
  if (invoice.template === 'elegant') {
    return (
      <div className="p-12 bg-white" style={{ 
        ...
        {invoice.paymentInfo?.qrCode && (
          <div className="flex justify-center mb-8">
            <div className="bg-white p-6 rounded-lg shadow-lg border border-amber-100 text-center">
              <h3 className="text-lg font-bold mb-4 text-amber-700">Payment QR Code</h3>
              <img
                src={invoice.paymentInfo.qrCode}
                alt="Payment QR Code"
    ...
    return (
      <div className="bg-white" style={{ 
        ...
        <div className="p-8">
          ...
          {invoice.paymentInfo?.qrCode && (
            <div className="flex justify-center mb-8">
              <div className="bg-gray-50 p-6 rounded-lg border-l-4 border-gray-900 text-center">
                <h3 className="font-bold text-gray-900 mb-3 uppercase tracking-wide">Payment QR Code</h3>
                <img
                  src={invoice.paymentInfo.qrCode}
                  alt="Payment QR Code"
                  className="w-32 h-32 mx-auto"
      ...
      <div className="bg-white relative overflow-hidden" style={{ 
        ...
        <div className="relative z-10 p-8">
          ...
          {invoice.paymentInfo?.qrCode && (
            <div className="flex justify-center mb-8">
              <div 
                className="p-6 rounded-2xl bg-gray-100 transform -rotate-1 shadow-lg border-2 text-center"
                style={{ borderColor: styles.accentColor }}
              >
                <h3 className="text-lg font-bold mb-4" style={{ color: styles.accentColor }}>
                  Payment QR Code
